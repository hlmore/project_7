...
plastic_regions_grouped <-
  ...
  summarize(meanLong=mean(na.omit(long)), 
            ...
            totalHard=sum(SUM_Hard_PlasticBeverageBottle, 
                          ...
                          SUM_OtherHardPlastic, 
                          na.rm=TRUE),
            totalSoft=sum(SUM_PlasticOrFoamPlatesBowlsCup,
                          SUM_Soft_Bag,
                          ...
                          na.rm=TRUE),
            totalMarine=sum(Fishing_Net,
                            SUM_FishingLineLureRope,
                            Fishing_BuoysAndFloats, 
                          na.rm=TRUE),
            totalOther=sum(SUM_HardOrSoft_PlasticBottleCap,
...
basemap %>%
  addMinicharts(
    ...
    type = "pie",
    chartdata = plastic_regions_grouped[, colnames(plastic_regions_grouped[4:7])],#c("totalStraw", "totalCig")],
    colorPalette = colors,
    legend = TRUE,
    showLabels = TRUE,
    width = 60
